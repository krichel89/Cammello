...
TRANSLATIONS = {
    ...
    '&Redo {what}': {
        'de': '&Wiederherstellen: {what}',
        'es': '&Rehacer: {what}',
        'fr': '&Rétablir : {what}',
        'it': '&Ripeti: {what}',
    },
    'Take back the last action - a bulk rejection comes back in one step.': {
        'de': 'Nimmt die letzte Aktion zurück – eine Massenablehnung kommt in einem Schritt zurück.',
        'es': 'Deshace la última acción: un rechazo masivo vuelve en un solo paso.',
        'fr': 'Annule la dernière action – un rejet en masse revient en une seule étape.',
        'it': 'Annulla l’ultima azione: un rifiuto di massa torna in un solo passo.',
    },
    'Put the action back that was just taken back.': {
        'de': 'Stellt die gerade zurückgenommene Aktion wieder her.',
        'es': 'Vuelve a aplicar la acción que se acaba de deshacer.',
        'fr': 'Rétablit l’action qui vient d’être annulée.',
        'it': 'Riapplica l’azione appena annullata.',
    },
    'Reject': {
        'de': 'Ablehnen',
        'es': 'Rechazar',
    ...
    'Clear rating': {
        'de': 'Bewertung entfernen',
        'es': 'Quitar valoración',
        'fr': 'Effacer la note',
        'it': 'Rimuovi valutazione',
    },
    'Rating {n}': {
        'de': 'Bewertung {n}',
        'es': 'Valoración {n}',
    ...
    'Clear colour': {
        'de': 'Farbmarke entfernen',
        'es': 'Quitar color',
        'fr': 'Effacer la couleur',
        'it': 'Rimuovi colore',
    },
    'Colour {name}': {
        'de': 'Farbe {name}',
    ...
    'Undone: {what}': {
        'de': 'Zurückgenommen: {what}',
        'es': 'Deshecho: {what}',
        'fr': 'Annulé : {what}',
        'it': 'Annullato: {what}',
    },
    'Redone: {what}': {
        'de': 'Wiederhergestellt: {what}',
        'es': 'Rehecho: {what}',
        'fr': 'Rétabli : {what}',
        'it': 'Ripristinato: {what}',
    },
    'Nothing left to redo.': {
        'de': 'Nichts mehr zum Wiederherstellen.',
        'es': 'No queda nada que rehacer.',
        'fr': 'Plus rien à rétablir.',
        'it': 'Non c’è più nulla da ripristinare.',
    },
    'Nothing to undo here - those images are not in this folder any more.': {
        'de': 'Hier gibt es nichts zurückzunehmen – diese Bilder sind nicht mehr in diesem Ordner.',
        'es': 'Aquí no hay nada que deshacer: esas imágenes ya no están en esta carpeta.',
        'fr': 'Rien à annuler ici – ces images ne sont plus dans ce dossier.',
        'it': 'Qui non c’è nulla da annullare: quelle immagini non sono più in questa cartella.',
    },
    'Nothing to redo here - those images are not in this folder any more.': {
        'de': 'Hier gibt es nichts wiederherzustellen – diese Bilder sind nicht mehr in diesem Ordner.',
        'es': 'Aquí no hay nada que rehacer: esas imágenes ya no están en esta carpeta.',
        'fr': 'Rien à rétablir ici – ces images ne sont plus dans ce dossier.',
        'it': 'Qui non c’è nulla da ripristinare: quelle immagini non sono più in questa cartella.',
    },
    'Undone: {what} - {n} image(s) were not reachable.': {
        'de': 'Zurückgenommen: {what} – {n} Bild(er) waren nicht erreichbar.',
        'es': 'Deshecho: {what}: {n} imagen(es) no estaban accesibles.',
        'fr': 'Annulé : {what} – {n} image(s) n’étaient pas accessibles.',
        'it': 'Annullato: {what} – {n} immagine/i non erano raggiungibili.',
    },
    'Rename': {
        'de': 'Umbenennen',
        'es': 'Renombrar',
    ...
    # ── Caption generation and loupe select-all (0.18.24) ────────────────
    'Generate captions': {
        'de': 'Bildunterschriften erzeugen',
        'es': 'Generar leyendas',
        'fr': 'Générer les légendes',
        'it': 'Genera didascalie',
    },
    '&Generate captions…': {
        'de': '&Bildunterschriften erzeugen…',
        'es': '&Generar leyendas…',
    ...
    'Fill the captions in every language from the depicts and the event, using the labels Wikidata already holds.': {
        'de': 'Füllt die Bildunterschriften in jeder Sprache aus dem Dargestellten und der Veranstaltung, mit den Bezeichnungen, die Wikidata schon hat.',
        'es': 'Rellena las leyendas en todos los idiomas a partir de lo representado y el evento, usando las etiquetas que Wikidata ya tiene.',
        'fr': 'Remplit les légendes dans toutes les langues à partir du sujet représenté et de l’événement, avec les libellés que Wikidata possède déjà.',
        'it': 'Compila le didascalie in ogni lingua dal soggetto raffigurato e dall’evento, usando le etichette che Wikidata già possiede.',
    },
    'A caption is built for each ticked language from the depicts and the event. Where a conjunction is known the caption reads as a sentence; otherwise it is the plain "Name, Event".': {
        'de': 'Für jede angehakte Sprache wird eine Bildunterschrift aus dem Dargestellten und der Veranstaltung gebaut. Wo eine Fügung bekannt ist, liest sie sich als Satz; sonst als schlichtes „Name, Veranstaltung".',
        'es': 'Para cada idioma marcado se construye una leyenda a partir de lo representado y el evento. Donde se conoce una conjunción, se lee como una frase; si no, como el simple «Nombre, Evento».',
        'fr': 'Pour chaque langue cochée, une légende est construite à partir du sujet représenté et de l’événement. Là où une conjonction est connue, elle se lit comme une phrase ; sinon, comme le simple « Nom, Événement ».',
        'it': 'Per ogni lingua selezionata viene costruita una didascalia dal soggetto raffigurato e dall’evento. Dove una congiunzione è nota, si legge come una frase; altrimenti come il semplice «Nome, Evento».',
    },
    'Languages': {
        'de': 'Sprachen',
        'es': 'Idiomas',
    ...
    'Conjunctions (edit to correct a sentence; leave blank for "Name, Event"):': {
        'de': 'Fügungen (zum Korrigieren eines Satzes bearbeiten; leer lassen für „Name, Veranstaltung"):',
        'es': 'Conjunciones (edite para corregir una frase; deje en blanco para «Nombre, Evento»):',
        'fr': 'Conjonctions (modifiez pour corriger une phrase ; laissez vide pour « Nom, Événement ») :',
        'it': 'Congiunzioni (modifica per correggere una frase; lascia vuoto per «Nome, Evento»):',
    },
    'Lang': {
        'de': 'Spr.',
        'es': 'Idm.',
    ...
    'Event form': {
        'de': 'Form der Veranstaltung',
        'es': 'Forma del evento',
        'fr': 'Forme de l’événement',
        'it': 'Forma dell’evento',
    },
    'Example': {
        'de': 'Beispiel',
        'es': 'Ejemplo',
    ...
    'Also write linked descriptions (Information templates with Wikipedia links)': {
        'de': 'Auch verlinkte Beschreibungen schreiben (Information-Vorlagen mit Wikipedia-Links)',
        'es': 'Escribir también descripciones enlazadas (plantillas Information con enlaces a Wikipedia)',
        'fr': 'Écrire aussi des descriptions liées (modèles Information avec liens Wikipédia)',
        'it': 'Scrivi anche descrizioni collegate (template Information con collegamenti a Wikipedia)',
    },
    'Write captions': {
        'de': 'Bildunterschriften schreiben',
        'es': 'Escribir leyendas',
        'fr': 'Écrire les légendes',
        'it': 'Scrivi didascalie',
    },
    '{n} caption(s) across {f} file(s) and {l} language(s).': {
        'de': '{n} Bildunterschrift(en) über {f} Datei(en) und {l} Sprache(n).',
        'es': '{n} leyenda(s) en {f} archivo(s) y {l} idioma(s).',
        'fr': '{n} légende(s) sur {f} fichier(s) et {l} langue(s).',
        'it': '{n} didascalia/e su {f} file e {l} lingua/e.',
    },
    'No depicts or "created during" found - fill those first, then generate the captions.': {
        'de': 'Kein „Dargestelltes" und kein „entstanden bei" gefunden – erst diese ausfüllen, dann die Bildunterschriften erzeugen.',
        'es': 'No se encontró «representa» ni «creado durante»: complete esos primero y luego genere las leyendas.',
        'fr': 'Aucun « dépeint » ni « créé lors de » trouvé – remplissez-les d’abord, puis générez les légendes.',
        'it': 'Nessun «raffigura» né «creato durante» trovato: compila prima quelli, poi genera le didascalie.',
    },
    'Captions written for {f} file(s) in {l} language(s).': {
        'de': 'Bildunterschriften für {f} Datei(en) in {l} Sprache(n) geschrieben.',
        'es': 'Leyendas escritas para {f} archivo(s) en {l} idioma(s).',
        'fr': 'Légendes écrites pour {f} fichier(s) en {l} langue(s).',
        'it': 'Didascalie scritte per {f} file in {l} lingua/e.',
    },
    'Select &all': {
        'de': '&Alle auswählen',
        'es': 'Seleccionar &todo',
    ...
    'Select every image passing the filter, to rate or send them all at once (also with Ctrl/Cmd-click in the filmstrip).': {
        'de': 'Alle Bilder auswählen, die durch den Filter kommen, um sie zusammen zu bewerten oder weiterzuschicken (auch mit Strg/Cmd-Klick im Filmstreifen).',
        'es': 'Seleccionar todas las imágenes que pasan el filtro, para valorarlas o enviarlas de una vez (también con Ctrl/Cmd-clic en la tira de imágenes).',
        'fr': 'Sélectionner toutes les images passant le filtre, pour les noter ou les envoyer en une fois (aussi par Ctrl/Cmd-clic dans le bandeau).',
        'it': 'Seleziona tutte le immagini che passano il filtro, per valutarle o inviarle in una volta (anche con Ctrl/Cmd-clic nella striscia).',
    },
}
